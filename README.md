...
- Some materials only use 4 of the 8 available LUT rows, and thus only have 1 RGBA layer in their IDMask array. The resulting PNG strips are square images. This poses a conundrum, because the png strips are all assumed to be 2 layers deep. If you have a square IDMask in your accurate shader, you should instead export the DDS version from the game, then import the mask to the shader by right clicking the unit and clicking "import IDMask". If you don't have any use for painting the IDMask, deselect "is SDF" when importing or the mask will either be unncessarily large or unnecessarily converter to/from sdf. This will import the IDMask as a 1-layer IDMask instead of assuming it is a 2-layer IDMask squished into a square.
